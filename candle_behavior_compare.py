...
behaviour, then also finds candle-only historical neighbours.
"""
...
from candle_similarity import CandleSimilarityConfig, candle_features, candle_sequence_similarity
...
def pivot_candle_index(structure: pd.DataFrame, position: int) -> int:
    return int(structure.iloc[position]["index"])


def sequence(df: pd.DataFrame, center: int, radius: int) -> pd.DataFrame:
    start = max(0, center - radius)
    end = min(len(df) - 1, center + radius)
    z = df.iloc[start : end + 1].copy()
    z["offset"] = np.arange(start - center, end - center + 1)
    return z
...
def score_historical_endpoints(
    ...
    for end in candidate_ends:
        ...
        if len(candidate) != needed:
            continue
        score = candle_sequence_similarity(current.drop(columns=["offset"], errors="ignore"),
                                            candidate.drop(columns=["offset"], errors="ignore"),
                                            config)
...
def plot_candles(ax, raw: pd.DataFrame, title: str, center: int) -> None:
    for i, row in enumerate(raw.itertuples()):
        ...
        ax.add_patch(plt.Rectangle((i - 0.32, lo), 0.64, height, fill=up, linewidth=0.6))
...
def plot_pairwise(rows: list[dict], output: Path, show: bool) -> None:
    ...
    for i, item in enumerate(rows):
        ...
        axes[i, 1].set_title(f"{item['title']} | candle similarity {item['candle_similarity']:.4f}")
...
def create(
    ...
    structural = find_similar_structures(structure, n_pivots=pivots, top_k=top_k,
    ...
    current_feature_window = sequence(features, current_center, radius)
    current_raw_window = sequence(raw, current_center, radius)

    candidate_ends = list(range(radius, max(radius, current_center - radius * 2) + 1))
    candle_only = score_historical_endpoints(features, current_center, radius, candidate_ends, sim_cfg, candle_top_k)
    ...
    for rank, (_, match) in enumerate(structural.iterrows(), 1):
        ...
        hist_feature_window = sequence(features, center, radius)
        hist_raw_window = sequence(raw, center, radius)
        score = candle_sequence_similarity(current_feature_window.drop(columns=["offset"], errors="ignore"),
                                            hist_feature_window.drop(columns=["offset"], errors="ignore"), sim_cfg)
        rows.append({
            ...
            "historical_features": hist_feature_window,
            "historical_raw": hist_raw_window,
        })

    comparison = pd.DataFrame([{k: v for k, v in r.items() if not k.startswith("historical_")} for r in rows])
    output_dir.mkdir(parents=True, exist_ok=True)
    comparison.to_csv(output_dir / "structural_vs_candle_similarity.csv", index=False)
    candle_only.to_csv(output_dir / "candle_only_top_matches.csv", index=False)
    ...
    for r in rows:
        pair_rows.append({
            ...
            "current": current_raw_window,
            "historical": r["historical_raw"],
        })
    ...
    plot_candles(axes[0, 0], current_raw_window, "CURRENT — raw candles", current_center)
    plot_feature_strip(axes[0, 1], current_feature_window, "CURRENT — candle anatomy")
    for i, (_, m) in enumerate(candle_only.iterrows(), 1):
        center = int(m["candidate_end"])
        z = sequence(raw, center, radius)
    ...
    fig.suptitle(f"{cfg.symbol} {timeframe} — Candle-only historical neighbours\nRaw OHLC and anatomy; descriptive research only.",
